...
$(VENV)/.testenv: $(VENV)/bin/activate
	...
	if . $(VENV_BIN)/activate && uv sync --help | grep -q -- "--active"; then \
		. $(VENV_BIN)/activate && uv sync --active --all-packages --reinstall --no-build-isolation --no-dev --no-group dev \
			--link-mode=copy; \
	else \
		. $(VENV_BIN)/activate && uv sync --all-packages --reinstall --no-build-isolation --no-dev --no-group dev  \
			--link-mode=copy; \
	fi
	cp .devcontainer/elibris.pth $(VENV)/lib/python3.11/site-packages
	touch $(VENV)/.testenv
...
.PHONY: fmt
fmt: setup ## Format Python code (only modified/added files)
ifneq ($(strip $(GIT_FILES)),)
	# Format code
	uv tool run ruff format $(GIT_FILES)
	# Sort imports
	uv tool run ruff check --select I --fix $(GIT_FILES)
	uv tool run ruff check --fix $(GIT_FILES)
else
	@echo "No modified/added Python files to format."
endif

.PHONY: fmt-check
fmt-check: setup ## Check Python code formatting and style without making changes (only modified/added files)
ifneq ($(strip $(GIT_FILES)),)
	uv tool run ruff format --check $(GIT_FILES)
	uv tool run ruff check --select I $(GIT_FILES)
	uv tool run ruff check $(GIT_FILES)
else
...
.PHONY: mypy
mypy: setup## Run mypy checks (only modified/added files)
ifneq ($(strip $(GIT_FILES)),)
